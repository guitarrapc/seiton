...
| `github-app-token-inputs` | ✓ | — | Error when known GitHub App token actions are missing permission-limiting inputs, or when owner-scoped app token issuance omits repository-limiting inputs (for example `actions/create-github-app-token` with `owner` but without `repositories`). |

...
| `deny_job_container_latest_image` | Detect `job.container.image` values pinned to mutable `:latest` tag (explicit or implicit latest) and forbid them to preserve deterministic execution and supply-chain stability. |
...
| `deny_job_container_latest_image` | Detects mutable `:latest` usage for `job.container.image` (including implicit latest) where runtime drift can bypass review-time expectations. | Pin container image to immutable digest (`@sha256:...`) or to approved versioned tags plus digest. | ✗ | Digest pinning still requires provenance/trust checks; combine with registry allowlist and update cadence policy. |

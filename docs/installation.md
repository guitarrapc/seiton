...
When you need to download a specific version, pass `--version`. The following example downloads `0.9.13`:

```sh
curl -fsSL https://raw.githubusercontent.com/guitarrapc/seiton/main/scripts/download.sh | bash -s -- --version 0.9.13
```
...
```sh
mkdir -p ./bin
curl -fsSL https://raw.githubusercontent.com/guitarrapc/seiton/main/scripts/download.sh | bash -s -- --version 0.9.13 --dir ./bin
```
...
docker pull ghcr.io/guitarrapc/seiton:0.9.13
docker pull ghcr.io/guitarrapc/seiton:v0.9.13
```

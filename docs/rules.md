...
Detects credential leakage risk when `actions/checkout` (without `persist-credentials: false`) is followed later in the same job by `actions/upload-artifact` uploading a dangerous path. Dangerous paths include repository-root and parent-directory forms such as `.`, `..`, `./**`, `**`, `${{ github.workspace }}`, `${{ github.workspace }}/**`, and `${{ github.workspace }}/..`. Equivalent bracket-form expressions such as `${{ github['workspace'] }}` and normalized equivalents such as `repo/..` or `${{ github.workspace }}/repo/..` are treated the same way.

...
- `ghcr.io/guitarrapc/seiton:1.6.0`
- `ghcr.io/guitarrapc/seiton:v1.6.0`

To confirm the image works:

```sh
docker run --rm ghcr.io/guitarrapc/seiton:v1.6.0 version
```

Lint all workflow files (read-only mount):

```sh
docker run --rm -v "$PWD:/repo:ro" ghcr.io/guitarrapc/seiton:v1.6.0
```

Lint a specific file:

```sh
docker run --rm -v "$PWD:/repo:ro" ghcr.io/guitarrapc/seiton:v1.6.0 .github/workflows/ci.yml
```

Apply fixes (omit `:ro` — writable mount is required):

```sh
docker run --rm -v "$PWD:/repo" ghcr.io/guitarrapc/seiton:v1.6.0 --fix
```
...
jobs:
  seiton:
    ...
    steps:
      ...
      - name: Run seiton in Docker
        run: docker run --rm -v "$PWD:/repo:ro" ghcr.io/guitarrapc/seiton:v1.6.0 --format sarif > seiton.sarif

...
- `ghcr.io/guitarrapc/seiton:0.9.25`
- `ghcr.io/guitarrapc/seiton:v0.9.25`

To confirm the image works:

```sh
docker run --rm ghcr.io/guitarrapc/seiton:v0.9.25 version
```

Lint all workflow files (read-only mount):

```sh
docker run --rm -v "$PWD:/repo:ro" ghcr.io/guitarrapc/seiton:v0.9.25
```

Lint a specific file:

```sh
docker run --rm -v "$PWD:/repo:ro" ghcr.io/guitarrapc/seiton:v0.9.25 .github/workflows/ci.yml
```

Apply fixes (omit `:ro` — writable mount is required):

```sh
docker run --rm -v "$PWD:/repo" ghcr.io/guitarrapc/seiton:v0.9.25 --fix
```
...
jobs:
  seiton:
    ...
    steps:
      ...
      - name: Run seiton in Docker
        run: docker run --rm -v "$PWD:/repo:ro" ghcr.io/guitarrapc/seiton:v0.9.25 --format sarif > seiton.sarif

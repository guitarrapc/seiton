...
| `artipacked` | Detect credential leakage risk when `actions/checkout` (without `persist-credentials: false`) is combined with `actions/upload-artifact` uploading a dangerous path (`.`, `..`, `${{ github.workspace }}`) in a way that may include hidden files. Root-like uploads can sweep sensitive checkout state into artifacts; for early pinned `actions/upload-artifact@v4.0` through `@v4.3`, hidden files are included by default, while for `actions/upload-artifact@v4.4+` and floating `@v4`, hidden files are excluded by default. As a result, legacy `.git/config` credentials are typically included by default for pinned `v4.0`–`v4.3`, but for `v4.4+`/floating `@v4` they are typically included only when hidden files are uploaded explicitly or hidden-file behavior is not statically known. Error for non-v6+ checkout; warning for v6+ (credentials stored in `$RUNNER_TEMP`). |

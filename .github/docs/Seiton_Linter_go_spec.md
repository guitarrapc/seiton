...
- The shared rule catalog includes opt-in informational rules `anonymous-definition` and `misfeature` in addition to the existing opt-in local and online rules. Go implementations must preserve the same rule IDs, default-off behavior, and default severity contract from `Seiton_Linter_spec.md` even if implementation work lands later.

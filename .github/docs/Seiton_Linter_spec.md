...
| `checkout-unsafe-pr` | ✓ | — | Warn when `actions/checkout` sets `allow-unsafe-pr-checkout` to `true` or to an expression in workflow steps or action-metadata composite steps. Missing or literal `false` values are safe. Literal `true` values are auto-fixable to `false`. |

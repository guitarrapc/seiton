...
| `checkout-unsafe-pr` | ✓ | — | Warn when `actions/checkout` sets `allow-unsafe-pr-checkout` to `true` or to an expression. Missing or literal `false` values are safe. Literal `true` values are auto-fixable to `false`. |

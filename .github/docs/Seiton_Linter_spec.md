...
- `action`: in addition to popular-action input checks and pinning hygiene, Seiton resolves **local** actions statically and validates input contracts and `runs.using` runner policy (`local-action-inputs`). Remaining gaps include full remote-action metadata depth, complete Docker action / uses-format edge-case breadth, and other non-local action metadata validation.

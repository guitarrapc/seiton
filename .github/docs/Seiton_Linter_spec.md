...
| `artipacked` | ✓ | — | Detect credential leakage risk when `actions/checkout` (without `persist-credentials: false`) is combined with `actions/upload-artifact` uploading a dangerous path (`.`, `..`, `${{ github.workspace }}`) in a way that may include hidden files. Root-like uploads can sweep sensitive checkout state into artifacts; for `actions/upload-artifact`, only floating `@v4` and pinned `v4.4+` exclude hidden files by default, while pinned `v4.0`–`v4.3` may include hidden files by default. Accordingly, legacy `.git/config` credentials are typically included only when hidden files are uploaded explicitly, when hidden-file behavior is not statically known, or when a pinned `v4.0`–`v4.3` release is used. Error for non-v6+ checkout; warning for v6+ (credentials stored in `$RUNNER_TEMP`). Independent of `checkout-persist-credentials`. |

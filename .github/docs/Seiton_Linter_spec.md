...
| `artipacked` | ✓ | — | Detect credential leakage risk when `actions/checkout` (without `persist-credentials: false`) is followed later in the same job by `actions/upload-artifact` uploading a dangerous path (`.`, `..`, `${{ github.workspace }}`) in a way that may include hidden files. Root-like uploads can sweep sensitive checkout state into artifacts; for `actions/upload-artifact`, only floating `@v4` and pinned `v4.4+` exclude hidden files by default, while pinned `v4.0`–`v4.3` include hidden files by default. Arbitrary or unrecognized version refs (e.g. `@v4-legacy`, `@v4.x`, `@v4.4-legacy`) are treated conservatively as unknown and assumed to potentially include hidden files. Accordingly, legacy `.git/config` credentials are typically included when a pinned `v4.0`–`v4.3` release is used, when hidden files are uploaded explicitly, or when hidden-file behavior is not statically known. Error for non-v6+ checkout; warning for v6+ (credentials stored in `$RUNNER_TEMP`). Independent of `checkout-persist-credentials`. |

...
| `impostor-commit` | ✗ | `online` | Error when a SHA-pinned `uses:` reference points to a commit that exists in the repository's object storage but is not reachable from any branch HEAD or tag in the referenced repository's own ref namespace. This detects both completely missing commits and fork-origin impostor commits. |

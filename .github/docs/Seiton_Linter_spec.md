...
| `artipacked` | ✓ | — | Detect credential leakage risk when `actions/checkout` (without `persist-credentials: false`) is followed later in the same job by `actions/upload-artifact` uploading a dangerous path (`.`, `..`, `./**`, `**`, `${{ github.workspace }}`, `${{ github.workspace }}/**`, `${{ github.workspace }}/..`, equivalent bracket-form expressions such as `${{ github['workspace'] }}`, or normalized equivalents such as `repo/..` and `${{ github.workspace }}/repo/..`) that may expose credentials. Root-like and parent-directory uploads can sweep sensitive checkout state into artifacts; for `actions/upload-artifact`, only floating `@v4` and pinned `v4.4+` exclude hidden files by default, while pinned `v4.0`–`v4.3` include hidden files by default. Arbitrary or unrecognized upload-artifact refs (e.g. `@v4-legacy`, `@v4.x`, `@v4.4-legacy`) are treated conservatively as unknown and assumed to potentially include hidden files. Checkout refs that do not parse as semantic versions (for example SHA pins, branch refs, `@v6-legacy`, `@v06`) are also treated conservatively as unknown and assumed to carry both legacy and v6+ risk buckets. Legacy checkout stores credentials in `.git/config`; v6+ stores them under `$RUNNER_TEMP`. Parent-directory uploads (e.g. `..`, `${{ github.workspace }}/..`) bypass the hidden-file filter check for v6+ because `$RUNNER_TEMP` is not a hidden directory. Error for non-v6+ checkout; warning for v6+ (credentials stored in `$RUNNER_TEMP`). Independent of `checkout-persist-credentials`. |
...
| `artipacked` | Detects dangerous artifact uploads after unsafe checkout in the same job. | `actions/checkout` without `persist-credentials: false` followed by `actions/upload-artifact` with a dangerous path such as `.`, `..`, `./**`, `**`, `${{ github.workspace }}`, `${{ github.workspace }}/**`, or `${{ github.workspace }}/..` (including equivalent bracket-form expressions such as `${{ github['workspace'] }}` and normalized equivalents such as `repo/..` or `${{ github.workspace }}/repo/..`). Legacy `.git/config` exposure depends on hidden-file behavior; explicit exclusions such as `!.git/**` or `!.git/config` suppress that legacy case. | Prevents root-like or parent-directory artifact uploads from sweeping persisted checkout credentials or other sensitive repository state into artifacts. | Set `with.persist-credentials: false` on checkout and restrict artifact uploads to explicit safe directories/files; for `actions/upload-artifact@v4+`, leave hidden files disabled unless they are truly required. | ✗ | With `actions/upload-artifact@v4`, hidden files are excluded by default, so legacy `.git/config` leakage generally requires hidden-file upload or non-default/unknown hidden-file behavior. Parent-directory uploads remain risky for `actions/checkout@v6+` because they can still reach `$RUNNER_TEMP`. Current limitation: `actions/checkout` `with.path` subdirectory correlation is not implemented, so later uploads of that checkout subdirectory are not diagnosed unless the upload path is otherwise root-like, parent-like, or workspace-like. |

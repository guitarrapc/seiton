...
| `artipacked` | ✓ | — | Detect credential leakage risk when `actions/checkout` (without `persist-credentials: false`) is combined with `actions/upload-artifact` uploading a dangerous path (`.`, `..`, `${{ github.workspace }}`). Root-like uploads can sweep sensitive checkout state into artifacts; with `actions/upload-artifact@v4`, hidden files are excluded by default, so legacy `.git/config` credentials are typically included only when hidden files are uploaded explicitly. Error for non-v6+ checkout; warning for v6+ (credentials stored in `$RUNNER_TEMP`). Independent of `checkout-persist-credentials`. |
...
| `artipacked` | Detects dangerous artifact uploads after unsafe checkout in the same job. | `actions/checkout` without `persist-credentials: false` followed by `actions/upload-artifact` with `path: .`, `..`, or `${{ github.workspace }}`. | Prevents root-like artifact uploads from sweeping persisted checkout credentials or other sensitive repository state into artifacts. | Set `with.persist-credentials: false` on checkout and restrict artifact uploads to explicit safe directories/files. | ✗ | With `actions/upload-artifact@v4`, hidden files are excluded by default, so legacy `.git/config` leakage generally also requires hidden-file upload or non-default behavior. Root-like uploads remain risky because they can still sweep other sensitive files or become unsafe if upload settings change. |

...
| `local-action-inputs` | ✓ | — | For statically resolvable local actions (`uses: ./...` / `../...`), validate `with:` inputs against parsed `action.yml`/`action.yaml`: unknown inputs, missing required inputs, deprecated inputs (warning), allowed `runs.using` values (`composite`, `docker`, `node20`, `node24`), and deprecated runners (`node12`, `node16`). |
...
| `dispatch-inputs` | ✓ | — | Validate `on.workflow_dispatch.inputs` schema: types, required flags, choice options and defaults, boolean/number default literals, duplicate options, and the maximum input count. |
| `schedule-event` | ✓ | — | Validate `schedule` event entries: five-field cron syntax, minimum interval (GitHub’s five-minute floor), and timezone strings where resolvable without full OS timezone database reliance. |
...
| `template-injection` | ✓ | — | Error when untrusted `github.event`-origin data is directly interpolated into `run` script sinks in unsafe ways. `env:` declarations are treated as indirection and are not reported by this rule. |
...
- `action`: in addition to popular-action input checks and pinning hygiene, Seiton resolves **local** actions statically and validates input contracts and `runs.using` runner policy (`local-action-inputs`). The parser also validates action metadata required keys (`description`, `runs`) and parses branding, inputs, and outputs sections. Remaining gaps include full remote-action metadata depth, complete Docker action / uses-format edge-case breadth, and other non-local action metadata validation.
...
- `workflow-call`: reusable-call job shape, `secrets: inherit` denial, and **local** `on.workflow_call` contract checking (`reusable-workflow`) are covered. Remote called-workflow contracts may still be incomplete without checking out the callee repository.
...
| `local-action-inputs` | Validates local action metadata contracts when `uses:` points into the repo (`./` / `../`). | Unknown `with:` key; missing required input; deprecated input; `runs.using: node16`; invalid `runs.using` value. | Matches actionlint-style local action checks: inputs and runner policy on statically resolvable actions. | Align `with:` with `action.yml` inputs; upgrade deprecated runners; fix `runs.using` to `composite`, `docker`, `node20`, or `node24`. | ✗ | Only local paths resolved from the workflow file are checked; remote actions rely on other rules (for example pinning, popular inputs). |
...
| `schedule-event` | Validates scheduled workflow cron and timezone hints. | Fewer than five cron fields; invalid ranges; interval under five minutes; dubious timezone string. | Prevents schedules that GitHub rejects or that run too frequently. | Fix cron expression, interval, and timezone per GitHub schedule rules. | ✗ | Cron semantics and DST behavior depend on GitHub’s scheduler; re-verify after edits. |

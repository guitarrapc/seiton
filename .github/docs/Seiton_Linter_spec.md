...
| `run-inputs-context-direct-use` | ✓ | — | Error when `run:` directly references `${{ inputs.* }}` or `${{ github.event.inputs.* }}`; must map via `env` (except no-expand contexts such as single-quoted shell strings and single-quoted heredocs). |

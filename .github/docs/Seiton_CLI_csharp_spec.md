...
Source bytes for snippet rendering are retained in a `Dictionary<string, byte[]>` source map (only allocated when text format without `--oneline` is active). Snippet lines are extracted as byte ranges (`LineSlice`: start/length within the source UTF-8 buffer) and written with `Utf8Writer.WriteLiteral` without per-line `string` materialization. Gutter/caret padding uses `Utf8Writer.WriteRepeated` / `WritePaddedDecimal` without intermediate `new string(...)` or interpolated strings. To avoid `WriteLine('|')` binding to `WriteLine(int)` (printing `124`), `Utf8Writer` provides a `WriteLine(char)` overload; the current gutter separator is emitted as the string literal `" |"` to align with source rows (`" N | "`). Caret alignment uses `SourceDisplayWidth` (`TabWidth = 4`, East Asian wide runes width 2) to map 1-based byte columns to terminal display columns without allocating strings.

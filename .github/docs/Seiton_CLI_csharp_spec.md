...
Source bytes for snippet rendering are retained in a `Dictionary<string, byte[]>` source map (only allocated when text format without `--oneline` is active). Snippet lines are extracted as byte ranges (`LineSlice`: start/length within the source UTF-8 buffer) and written with `Utf8Writer.WriteLiteral` without per-line `string` materialization. Gutter/caret padding uses `Utf8Writer.WriteRepeated` / `WritePaddedDecimal` without intermediate `new string(...)` or interpolated strings. Gutter separator pipes use `Utf8Writer.WriteLine(char)`; this overload must exist because `WriteLine('|')` without it resolves to `WriteLine(int)` and emits the ASCII code as decimal text (`124`). Caret alignment uses `SourceDisplayWidth` (`TabWidth = 4`, East Asian wide runes width 2) to map 1-based byte columns to terminal display columns without allocating strings.

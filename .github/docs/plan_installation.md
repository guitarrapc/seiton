...
- [`.github/workflows/release.yaml`](../../.github/workflows/release.yaml) の **`docker` ジョブ**（`needs: validate, publish`）:
  ...
  - タグ: `:latest`, `:<Version>`, `:<tag>`（例 `v1.3.0`）
...
- **タグ衝突の回避**: CLI は固定 semver タグ (`v1.3.0`) でリリースするが、Action の慣例は floating major タグ (`v1`) を最新 `v1.x.x` に追従させる。同一リポでは両タグ体系が干渉しリリースフローが複雑化する。
...
- タグ運用: リリース時に `v1.3.0` タグを打ち、`v1` floating tag を追従させる。

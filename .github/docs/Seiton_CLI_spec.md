...
Mermaid `flowchart LR` text to stdout: each job is a subgraph with chained step nodes, transitively reduced `needs` edges (`reducedNeeds`) connect job subgraphs, `parallel` boundaries are nested subgraphs whose children are intentionally unchained (simultaneous), and reusable jobs are subroutine nodes (`[[...]]`). Labels are single-line, quote-sanitized, and truncated. A matrix with non-empty static `combinations` is labeled `(matrix: N)`, where `N` is the expanded combination count; a whole-value matrix expression is labeled `(matrix: dynamic)`, and a matrix without statically available combinations is labeled `(matrix)`.
...
- **Live-AST and owned-DTO serialization are equivalent contracts**: both `flow-json` and `flow-mermaid` must produce byte-identical output through either path for the same workflow. Differential tests cover representative structure and matrix variants, including include-only and dynamic matrices, because the Playground uses the live-AST path while other callers may use the owned DTO.

...
- 単一引用符内の診断抑制を必要に応じてオプトインで解除できるようにする。

対応:

1. run-inputs-context-direct-useとrun-env-context-direct-useについて、コンフィグでstrictオプションを用意してする。
2. 仕様書、configuration.md、rules.mdドキュメントの更新を行う

完了条件:

- strictオプションが無効な場合、単一引用符内の診断が抑制されることをテストで確認する
- strictオプションが有効な場合、単一引用符内の診断が復活することをテストで確認する
- strictオプションの説明がドキュメントに明確に記載されていること

### P5: クロスルール整合とガードレール整備

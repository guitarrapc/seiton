...
### 6.4 Flow Tab: LOD Layout vs Display Zoom

Zoom scale selects the LOD tier (lod0/lod1/lod2). Each tier precomputes its own job-card layout (compact at lod0, tighter at lod1, full at lod2) so panning out reduces **information**, not on-screen text size. Crossing a LOD threshold reflows job positions and edge paths; zoom toolbar buttons still scale the whole SVG for display size. Step inner coordinates are drawn once at lod2 resolution and scaled inside the card at lod1.

...
**Curated policy merge pattern (`permissions`)**: GitHub removes a permission scope from the docs table when it retires the underlying feature, but the Actions runner keeps accepting the scope. Dropping it from the known set would report `unknown permission scope` on workflows that still run, so Stage 3 re-adds such scopes to the canonical snapshot and marks them with a `deprecationNote`. Being absent from the docs table and being retired are separate facts, curated separately: the note is applied whether the scope came from the docs table or from the re-add list, so a scope reappearing in the docs does not silently lose it. The generated `PermissionScopes.GetDeprecationNote` drives the `deprecated-permissions` rule while `IsKnownScope` / `GetAllowedValues` keep treating the scope as valid. Unlike the datasets above, both curated lists live in the fetcher rather than in a `data/` JSON file: the deprecation notes are user-facing message text owned by the linter, and the re-add list is kept next to them so a scope's two facts stay in one place.

...
| P1-3 | **`network.max-concurrency` の上限** | 例: **1〜32 または 64** のキャップ。超過はクランプまたはエラー。 |
...
- [ ] P0: `ghes-api-url` が `http://` 等のとき **検証失敗**すること。
- [ ] P0: リダイレクト挙動とトークンヘッダの組み合わせを **意図どおり**にテスト（モックサーバまたは記録済みレスポンス）。
- [ ] P1: 悪意ある `ignore-actions` パターンで **プロセスが事実上固まらない**（タイムアウトまたは拒否）。
- [ ] P1: 巨大設定ファイル・巨大 `exclusions` リストで **明確にエラー**または **上限内に収まる**。

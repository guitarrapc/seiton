...
| Diagnostics | Grouped per file on stdout with `::group::<file>` / `::endgroup::`; body is rich text by default or one-line with `--oneline`; displayed file paths are escaped for `%`, `\r`, `\n`; no color. |

...
| `artipacked` | Detect credential leakage risk when `actions/checkout` (without `persist-credentials: false`) is followed later in the same job by `actions/upload-artifact` uploading a dangerous path (`.`, `..`, `../../_temp`, `../../_temp/**`, `../../_temp/*`, `../../_temp/**/*`, `./**`, `./**/*`, `**`, `**/*`, `${{ github.workspace }}`, `${{ github.workspace }}/**`, `${{ github.workspace }}/**/*`, `${{ github.workspace }}/..`, `${{ github.workspace }}/../../_temp`, and equivalent bracket-form expressions such as `${{ github['workspace'] }}`) that can capture checkout-managed credentials. Core intent: prevent artifact uploads from sweeping checkout-managed credentials or adjacent sensitive state into artifacts. Current supported scope: normalized equivalents such as `repo/..` and `${{ github.workspace }}/repo/..` are also treated as dangerous when they resolve back to the workspace root, and explicit escaped `_temp` directory paths and their glob variants (e.g. `../../_temp`, `../../_temp/**`, `../../_temp/*`, `${{ github.workspace }}/../../_temp`) are treated as dangerous because they can reach `$RUNNER_TEMP` on GitHub-hosted runners. Workspace-expression suffixes are recognized only when the suffix is empty or begins with `/` or `\`, so concatenations such as `${{ github.workspace }}..` or `!${{ github.workspace }}.git/**` are not treated as workspace-relative paths. For legacy non-v6+ checkout, this is reported when the upload occurs in a way that may include hidden files, because root-like uploads can sweep `.git/config` into artifacts; for `actions/upload-artifact`, only pinned `v4.4+` and floating `@v4` are treated as excluding hidden files by default, while pinned `v4.0`–`v4.3` are treated as including hidden files by default (the `include-hidden-files` input did not exist in those versions). Arbitrary or unrecognized upload-artifact refs (e.g. `@v4-legacy`, `@v4.x`, `@v4.4-legacy`) are treated conservatively as unknown unless `include-hidden-files` is set to a static boolean, in which case that explicit value is respected. Checkout refs that do not parse as semantic versions (for example SHA pins, branch refs, `@v6-legacy`, `@v06`) are also treated conservatively as unknown and assumed to carry both legacy and v6+ risk buckets. As a result, legacy `.git/config` credentials are typically included when the upload-artifact version is pinned to `v4.0`–`v4.3`, when hidden files are uploaded explicitly via `include-hidden-files: true`, or when hidden-file behavior is not statically known. Explicit exclusion lines such as `!.git/**`, `!.git/config`, explicit nested exclusions such as `!repo/.git/**`, and leading-recursive forms such as `!**/.git/**` suppress the legacy `.git/config` case only when they exclude every reachable `.git/config` location within the current normalized-path depth bound; bare `!.git` never suppresses. Leading-recursive exclusions (`!**/.git/**`) suppress the legacy case even when the checkout `with.path` contains runtime expressions that cannot be statically normalized. For v6+ checkout, credentials are stored in `$RUNNER_TEMP`, and the implementation intentionally bypasses the hidden-files check only for parent-directory uploads that can reach `$RUNNER_TEMP`; these cases are still reported as warnings even if hidden files would otherwise be excluded. Error for non-v6+ checkout; warning only for v6+ parent-directory uploads that can reach `$RUNNER_TEMP`. Implemented as a step-order scan in `VisitJobPost`: it tracks whether unsafe legacy or v6+ checkout has already run, reports only later dangerous upload-artifact `path` values, and when legacy exclusion lines are present it re-evaluates the preceding unsafe legacy checkouts against that upload path to decide whether every reachable `.git/config` location is excluded. Deferred scope: `actions/checkout` `with.path` subdirectory correlation is not implemented beyond explicit nested `.git` exclusions, so later uploads of that checkout subdirectory are not diagnosed unless the upload path is otherwise root-like, parent-like, or workspace-like. Independent of `checkout-persist-credentials`. |

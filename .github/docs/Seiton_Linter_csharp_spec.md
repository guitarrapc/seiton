...
- `action`: covered by `popular-action-inputs` / `outdated-action-runner` / `unpinned-uses` / `local-action-inputs`. `popular-action-inputs` validates input names against catalog; `outdated-action-runner` flags deprecated `runs.using` runtimes via catalog `GetRunsUsing()`; `local-action-inputs` validates local action contracts and runner policy. Still missing full remote-action metadata depth, complete Docker action / uses-format edge-case breadth, and other non-local action metadata validation.

...
| `checkout-unsafe-pr` | Implemented as a `VisitStep` checkout input check. Literal `true` values are fixable to `false`; expression values emit diagnostics without fixes. |

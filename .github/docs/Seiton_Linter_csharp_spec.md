...
| `artipacked` | Detect credential leakage risk when `actions/checkout` (without `persist-credentials: false`) is followed later in the same job by `actions/upload-artifact` uploading a dangerous path (`.`, `..`, `${{ github.workspace }}`) in a way that may include hidden files. Root-like uploads can sweep sensitive checkout state into artifacts; for `actions/upload-artifact`, only pinned `v4.4+` and floating `@v4` are treated as excluding hidden files by default, while pinned `v4.0`-`v4.3` are treated as potentially including hidden files unless hidden-file behavior is explicit or otherwise statically known. As a result, legacy `.git/config` credentials are typically included only when hidden files are uploaded explicitly or hidden-file behavior is not statically known. Error for non-v6+ checkout; warning for v6+ (credentials stored in `$RUNNER_TEMP`). Implemented as a zero-allocation single-pass scan over `job.Steps` in `VisitJobPost`, tracking whether an unsafe checkout has already run (legacy vs v6+) and reporting only later dangerous upload-artifact `path` values. Independent of `checkout-persist-credentials`. |

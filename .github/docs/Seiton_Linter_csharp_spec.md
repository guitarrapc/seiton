...
| `artipacked` | Detect credential leakage when `actions/checkout` (without `persist-credentials: false`) is combined with `actions/upload-artifact` uploading a dangerous path (`.`, `..`, `${{ github.workspace }}`). Error for non-v6+ checkout; warning for v6+ (credentials stored in `$RUNNER_TEMP`). Implemented as a zero-allocation two-pass scan over `job.Steps` in `VisitJobPost`: first determine whether the job contains any unsafe checkout (legacy vs v6+), then report each dangerous upload-artifact `path` at the path value location. Independent of `checkout-persist-credentials`. |

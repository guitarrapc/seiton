...
- `ActionRefResolution` includes `IsReachable` (bool): true when the commit is reachable from at least one branch HEAD or tag in the repository's own ref namespace. Determined by `branches-where-head` API when `IsTaggedCommit` is false. The `impostor-commit` rule uses this to detect fork-origin commits that exist in the repository's shared object storage but are not reachable from any legitimate branch or tag.

...
| `artipacked` | Detect credential leakage risk when `actions/checkout` (without `persist-credentials: false`) is followed later in the same job by `actions/upload-artifact` uploading a dangerous path (`.`, `..`, `./**`, `./**/*`, `**`, `**/*`, `${{ github.workspace }}`, `${{ github.workspace }}/**`, `${{ github.workspace }}/**/*`, `${{ github.workspace }}/..`, and equivalent bracket-form expressions such as `${{ github['workspace'] }}`) that can capture checkout-managed credentials. Normalized equivalents such as `repo/..` and `${{ github.workspace }}/repo/..` are also treated as dangerous when they resolve back to the workspace root. For legacy non-v6+ checkout, this is reported when the upload occurs in a way that may include hidden files, because root-like uploads can sweep `.git/config` into artifacts; for `actions/upload-artifact`, only pinned `v4.4+` and floating `@v4` are treated as excluding hidden files by default, while pinned `v4.0`–`v4.3` are treated as including hidden files by default (the `include-hidden-files` input did not exist in those versions). Arbitrary or unrecognized upload-artifact refs (e.g. `@v4-legacy`, `@v4.x`, `@v4.4-legacy`) are treated conservatively as unknown and assumed to potentially include hidden files. Checkout refs that do not parse as semantic versions (for example SHA pins, branch refs, `@v6-legacy`, `@v06`) are also treated conservatively as unknown and assumed to carry both legacy and v6+ risk buckets. As a result, legacy `.git/config` credentials are typically included when the upload-artifact version is pinned to `v4.0`–`v4.3`, when hidden files are uploaded explicitly via `include-hidden-files: true`, or when hidden-file behavior is not statically known. Explicit exclusion lines such as `!.git/**` or `!.git/config` suppress the legacy `.git/config` case. For v6+ checkout, credentials are stored in `$RUNNER_TEMP`, and the implementation intentionally bypasses the hidden-files check only for parent-directory uploads that can reach `$RUNNER_TEMP`; these cases are still reported as warnings even if hidden files would otherwise be excluded. Error for non-v6+ checkout; warning only for v6+ parent-directory uploads that can reach `$RUNNER_TEMP`. Implemented as a zero-allocation single-pass scan over `job.Steps` in `VisitJobPost`, tracking whether an unsafe checkout has already run (legacy vs v6+) and reporting only later dangerous upload-artifact `path` values. Current limitation: `actions/checkout` `with.path` subdirectory correlation is not implemented, so later uploads of that checkout subdirectory are not diagnosed unless the upload path is otherwise root-like, parent-like, or workspace-like. Independent of `checkout-persist-credentials`. |
| `artipacked` | Detect credential leakage risk when `actions/checkout` (without `persist-credentials: false`) is followed later in the same job by `actions/upload-artifact` uploading a dangerous path (`.`, `..`, `./**`, `./**/*`, `**`, `**/*`, `${{ github.workspace }}`, `${{ github.workspace }}/**`, `${{ github.workspace }}/**/*`, `${{ github.workspace }}/..`, and equivalent bracket-form expressions such as `${{ github['workspace'] }}`) that can capture checkout-managed credentials. Core intent: prevent artifact uploads from sweeping checkout-managed credentials or adjacent sensitive state into artifacts. Current supported scope: normalized equivalents such as `repo/..` and `${{ github.workspace }}/repo/..` are also treated as dangerous when they resolve back to the workspace root; for legacy non-v6+ checkout, this is reported when the upload occurs in a way that may include hidden files, because root-like uploads can sweep `.git/config` into artifacts; for `actions/upload-artifact`, only pinned `v4.4+` and floating `@v4` are treated as excluding hidden files by default, while pinned `v4.0`–`v4.3` are treated as including hidden files by default (the `include-hidden-files` input did not exist in those versions). Arbitrary or unrecognized upload-artifact refs (e.g. `@v4-legacy`, `@v4.x`, `@v4.4-legacy`) are treated conservatively as unknown and assumed to potentially include hidden files. Checkout refs that do not parse as semantic versions (for example SHA pins, branch refs, `@v6-legacy`, `@v06`) are also treated conservatively as unknown and assumed to carry both legacy and v6+ risk buckets. As a result, legacy `.git/config` credentials are typically included when the upload-artifact version is pinned to `v4.0`–`v4.3`, when hidden files are uploaded explicitly via `include-hidden-files: true`, or when hidden-file behavior is not statically known. Explicit exclusion lines such as `!.git/**` or `!.git/config` suppress the legacy `.git/config` case. For v6+ checkout, credentials are stored in `$RUNNER_TEMP`, and the implementation intentionally bypasses the hidden-files check only for parent-directory uploads that can reach `$RUNNER_TEMP`; these cases are still reported as warnings even if hidden files would otherwise be excluded. Error for non-v6+ checkout; warning only for v6+ parent-directory uploads that can reach `$RUNNER_TEMP`. Implemented as a zero-allocation single-pass scan over `job.Steps` in `VisitJobPost`, tracking whether an unsafe checkout has already run (legacy vs v6+) and reporting only later dangerous upload-artifact `path` values. Deferred scope: `actions/checkout` `with.path` subdirectory correlation is not implemented, so later uploads of that checkout subdirectory are not diagnosed unless the upload path is otherwise root-like, parent-like, or workspace-like. Independent of `checkout-persist-credentials`. |

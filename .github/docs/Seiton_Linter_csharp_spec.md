...
- Canonical tag-comment promotion (`fix.pinning.prefer-canonical-tag-comment`, default `true`) applies to alias-like version refs (`vN`, `vN.M`): resolver may map resolved SHA back to the highest compatible concrete tag (for example `v1` -> `v1.0.2`). Promotion is skipped for concrete patch refs (`vN.M.P`) to avoid unnecessary API calls.

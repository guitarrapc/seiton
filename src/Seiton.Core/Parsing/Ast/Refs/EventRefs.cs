...
public readonly struct EventRef
{
    ...
    public EventKind Kind => HasValue ? ArenaChecked!.GetEvent(_raw - 1).Kind : EventKind.None;

    public StringRef EventName => HasValue ? new(ArenaChecked, ArenaChecked!.GetEvent(_raw - 1).EventName) : default;

    public TextRange Range => HasValue ? ArenaChecked!.GetEvent(_raw - 1).Range : default;

    private int Payload => HasValue ? ArenaChecked!.GetEvent(_raw - 1).Payload : 0;

    /// <summary>The webhook payload. Default when <see cref="Kind"/> is not <see cref="EventKind.Webhook"/>.</summary>
    public WebhookEventRef AsWebhook() => Kind == EventKind.Webhook && Payload > 0 ? new(ArenaChecked, Payload) : default;

    /// <summary>The schedule payload. Default when <see cref="Kind"/> is not <see cref="EventKind.Scheduled"/>.</summary>
    public ScheduledEventRef AsScheduled() => Kind == EventKind.Scheduled && Payload > 0 ? new(ArenaChecked, Payload) : default;

    /// <summary>The dispatch payload. Default when <see cref="Kind"/> is not <see cref="EventKind.WorkflowDispatch"/>.</summary>
    public WorkflowDispatchEventRef AsWorkflowDispatch() => Kind == EventKind.WorkflowDispatch && Payload > 0 ? new(ArenaChecked, Payload) : default;

    /// <summary>The workflow_call payload. Default when <see cref="Kind"/> is not <see cref="EventKind.WorkflowCall"/>.</summary>
    public WorkflowCallEventRef AsWorkflowCall() => Kind == EventKind.WorkflowCall && Payload > 0 ? new(ArenaChecked, Payload) : default;

    /// <summary>The repository_dispatch payload. Default when <see cref="Kind"/> is not <see cref="EventKind.RepositoryDispatch"/>.</summary>
    public RepositoryDispatchEventRef AsRepositoryDispatch() => Kind == EventKind.RepositoryDispatch && Payload > 0 ? new(ArenaChecked, Payload) : default;

    /// <summary>The image_version payload. Default when <see cref="Kind"/> is not <see cref="EventKind.ImageVersion"/>.</summary>
    public ImageVersionEventRef AsImageVersion() => Kind == EventKind.ImageVersion && Payload > 0 ? new(ArenaChecked, Payload) : default;
}
...
public readonly struct WebhookEventRef
{
    ...
    public StringRef Hook => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookEvent(_payload).Hook) : default;

    public StringRefList Types => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookEvent(_payload).Types) : default;

    public WebhookEventFilterRef Branches => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookEvent(_payload).Branches) : default;

    public WebhookEventFilterRef BranchesIgnore => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookEvent(_payload).BranchesIgnore) : default;

    public WebhookEventFilterRef Tags => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookEvent(_payload).Tags) : default;

    public WebhookEventFilterRef TagsIgnore => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookEvent(_payload).TagsIgnore) : default;

    public WebhookEventFilterRef Paths => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookEvent(_payload).Paths) : default;

    public WebhookEventFilterRef PathsIgnore => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookEvent(_payload).PathsIgnore) : default;

    public StringRefList Workflows => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookEvent(_payload).Workflows) : default;
}
...
public readonly struct WebhookEventFilterRef
{
    ...
    public StringRef Name => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookFilter(_id).Name) : default;

    public StringRefList Values => HasValue ? new(ArenaChecked, ArenaChecked!.GetWebhookFilter(_id).Values) : default;
}
...
public readonly struct ScheduledEventRef
{
    ...
    public ScheduleRefList Schedules => HasValue ? new(ArenaChecked, ArenaChecked!.GetScheduledEvent(_payload).Schedules) : default;
}
...
public readonly struct ScheduleEntryRef
{
    ...
    public StringRef Cron => new(ArenaChecked, _row.Cron);

    public StringRef Timezone => new(ArenaChecked, _row.Timezone);
...
public readonly struct WorkflowDispatchEventRef
{
    ...
    public DispatchInputRefMap Inputs => HasValue ? new(ArenaChecked, ArenaChecked!.GetWorkflowDispatchEvent(_payload).Inputs) : default;
}
...
public readonly struct DispatchInputRef
{
    ...
    public StringRefList Options => new(ArenaChecked, _row.Options);
...
public readonly struct WorkflowCallEventRef
{
    ...
    public WorkflowCallEventInputRefList Inputs => HasValue ? new(ArenaChecked, ArenaChecked!.GetWorkflowCallEvent(_payload).Inputs) : default;

    public WorkflowCallEventSecretRefMap Secrets => HasValue ? new(ArenaChecked, ArenaChecked!.GetWorkflowCallEvent(_payload).Secrets) : default;

    public WorkflowCallEventOutputRefMap Outputs => HasValue ? new(ArenaChecked, ArenaChecked!.GetWorkflowCallEvent(_payload).Outputs) : default;
}
...
public readonly struct RepositoryDispatchEventRef
{
    ...
    public StringRefList Types => HasValue ? new(ArenaChecked, ArenaChecked!.GetRepositoryDispatchEvent(_payload).Types) : default;
}
...
public readonly struct ImageVersionEventRef
{
    ...
    public StringRefList Names => HasValue ? new(ArenaChecked, ArenaChecked!.GetImageVersionEvent(_payload).Names) : default;

    public StringRefList Versions => HasValue ? new(ArenaChecked, ArenaChecked!.GetImageVersionEvent(_payload).Versions) : default;
}

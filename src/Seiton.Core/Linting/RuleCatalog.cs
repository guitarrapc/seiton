...
internal static class RuleCatalog
{
    ...
    [
        ...
        ("deny-write-all", 14, static () => new DenyWriteAllRule()),
        ("credentials", 15, static () => new CredentialsRule()),
        ("template-injection", 16, static () => new TemplateInjectionRule()),
        ("expr-undefined-var", 17, static () => new ExprUndefinedVarRule()),
        ("run-env-context-direct-use", 18, static () => new RunEnvContextDirectUseRule()),
        ("runner-no-latest", 19, static () => new RunnerNoLatestRule()),
        ("run-secrets-context-direct-use", 20, static () => new RunSecretsContextDirectUseRule()),
        ("run-inputs-context-direct-use", 21, static () => new RunInputsContextDirectUseRule()),
        ("secrets-whole-context-access", 22, static () => new SecretsWholeContextAccessRule()),
        ("checkout-persist-credentials", 23, static () => new CheckoutPersistCredentialsRule()),
        ("deny-read-all", 24, static () => new DenyReadAllRule()),
        ("deny-inherit-secrets", 25, static () => new DenyInheritSecretsRule()),
        ("job-timeout-minutes-required", 26, static () => new JobTimeoutMinutesRequiredRule()),
        ("github-app-token-inputs", 27, static () => new GitHubAppTokenInputsRule()),
        ("cache-poisoning", 32, static () => new CachePoisoningRule()),
        ("self-hosted-runner", 33, static () => new SelfHostedRunnerRule()),
        ("unredacted-secrets", 34, static () => new UnredactedSecretsRule()),
        ("secrets-outside-env", 35, static () => new SecretsOutsideEnvRule()),
        ("workflow-secrets", 36, static () => new WorkflowSecretsRule()),
        ("job-secrets", 37, static () => new JobSecretsRule()),
        ("action-shell-is-required", 38, static () => new ActionShellIsRequiredRule()),
        ("matrix", 39, static () => new MatrixRule()),
        ("env-var", 40, static () => new EnvVarRule()),
        ("deprecated-commands", 41, static () => new DeprecatedCommandsRule()),
        ("if-cond", 42, static () => new IfCondRule()),
        ("fake-ternary", 43, static () => new FakeTernaryRule()),
    ...
    [
        ("known-vulnerable-actions", 28),
        ("impostor-commit", 29),
        ("ref-confusion", 30),
        ("stale-action-refs", 31),
    ];

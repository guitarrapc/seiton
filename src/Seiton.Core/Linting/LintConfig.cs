...
public sealed record RuleConfig
{
    ...
    /// <summary>Gets the additional trigger list for <c>cache-poisoning-trigger</c> and <c>self-hosted-runner-trigger</c> (appended to each rule's built-in set).</summary>
    ...
    /// <summary>
    /// Gets whether strict mode is enabled for rules that support no-expand-context diagnostics.
    /// Defaults to <c>false</c>.
    /// </summary>

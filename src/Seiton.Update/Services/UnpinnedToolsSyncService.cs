...
internal sealed class UnpinnedToolsSyncService
{
    private readonly UnpinnedToolsSourceParser parser = new();
    private readonly UnpinnedToolsCSharpGenerator generator = new();

    public bool Sync(string repoRoot)
    {
        var primarySourcePath = UnpinnedToolsSourcePathResolver.ResolvePrimary(repoRoot);
        var outputPath = Path.Combine(repoRoot, "src", "Seiton.Core", "Generated", "UnpinnedToolsActions.g.cs");

...
internal sealed class SuperfluousActionsSyncService
{
    private readonly SuperfluousActionsSourceParser parser = new();
    private readonly SuperfluousActionsCSharpGenerator generator = new();

    public bool Sync(string repoRoot)
    {
        var primarySourcePath = SuperfluousActionsSourcePathResolver.ResolvePrimary(repoRoot);
        var outputPath = Path.Combine(repoRoot, "src", "Seiton.Core", "Generated", "SuperfluousActions.g.cs");

...
internal sealed class ContextTypesSyncService
{
    private readonly ContextTypesSourceParser parser = new();
    private readonly ContextTypesCSharpGenerator generator = new();

    public bool Sync(string repoRoot)
    {
        var sourcePath = ContextTypesSourcePathResolver.ResolvePrimary(repoRoot);
        var outputPath = Path.Combine(repoRoot, "src", "Seiton.Core", "Generated", "ContextTypes.g.cs");

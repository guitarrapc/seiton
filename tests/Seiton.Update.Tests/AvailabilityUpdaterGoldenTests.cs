...
public sealed class AvailabilityUpdaterGoldenTests
{
    [Test]
    public async Task Generate_FromGitHubPrimarySource_MatchesAvailabilityGoldenFile()
    {
        var repoRoot = FindRepoRoot();
        var sourcePath = Path.Combine(repoRoot, "data", "sources", "availability", "github", "availability.json");
        var goldenPath = Path.Combine(repoRoot, "src", "Seiton.Core", "Generated", "Availability.g.cs");

        var parser = new GitHubAvailabilitySourceParser();
        var generator = new AvailabilityCSharpGenerator();
    ...
    public async Task Sync_WhenOutputAlreadyCurrent_IsIdempotent()
    {
        ...
        try
        {
            var sync = new AvailabilitySyncService();
    ...
    public async Task Verify_WhenGeneratedFileIsStale_ReturnsFalseThenSyncRepairs()
    {
        ...
        try
        {
            var outputPath = Path.Combine(tempRepo, "src", "Seiton.Core", "Generated", "Availability.g.cs");
    ...
    static string CreateTempRepoCopy(string repoRoot)
    {
        ...
        Directory.CreateDirectory(Path.Combine(tempRepo, "data", "sources", "availability", "github"));
        ...
        File.Copy(
            Path.Combine(repoRoot, "src", "Seiton.Core", "Generated", "Availability.g.cs"),
            Path.Combine(tempRepo, "src", "Seiton.Core", "Generated", "Availability.g.cs"),
            overwrite: true);

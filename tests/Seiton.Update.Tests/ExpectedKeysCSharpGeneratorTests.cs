...
public sealed class ExpectedKeysCSharpGeneratorTests
{
    ...
    public async Task Generate_CommittedModel_EmitsJobMappingKeyTableAndMetadata()
    {
        ...
        var runsOnOrdinal = -1;
        for (var i = 0; i < jobSection.Keys.Count; i++)
        {
            if (jobSection.Keys[i] == "runs-on")
            {
                runsOnOrdinal = i;
                break;
            }
        }
        await Assert.That(runsOnOrdinal).IsGreaterThanOrEqualTo(0);
        ...
        await Assert.That(output).Contains($"public static int KeyCount => {jobSection.Keys.Count};");
        await Assert.That(output).Contains($"RunsOn = {runsOnOrdinal},");

...
public sealed class AgenticWorkflowSkipTests
{
    ...
    [Test]
    public async Task ResolveFiles_SkipAgenticWorkflows_DoNotEditWithoutMetadata_IsNotSkipped()
    {
        ...
        File.WriteAllText(
            Path.Combine(workflowsDir, "agentics-maintenance.yml"),
            "# DO NOT EDIT - generated by gh-aw\non: push\n");

        try
        {
            var logger = VerboseLogger.Create(VerboseLevel.Off, TextWriter.Null);
            ...
            await Assert.That(files.Any(f => f.Contains("agentics-maintenance.yml", StringComparison.Ordinal))).IsTrue();
        }
        finally
        {

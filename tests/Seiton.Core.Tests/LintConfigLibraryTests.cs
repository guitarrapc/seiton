...
public sealed class LintConfigLibraryTests
{
    ...
    [Test]
    public async Task Validate_BotConditions_StrictDetection_ExplicitFalse_Parses()
    {
        var yaml = """
        rules:
          bot-conditions:
        ...
        await Assert.That(result.IsValid).IsTrue();
        await Assert.That(result.Config!.Rules!["bot-conditions"].StrictDetection).IsFalse();
    }

    [Test]
    public async Task Validate_BotConditions_StrictDetection_InvalidValue_ReturnsError()
    {
        var yaml = """
        rules:
          bot-conditions:
            strict-detection: maybe
        """;

        var result = LintConfigLibrary.Validate(yaml, "seiton.yaml");

        await Assert.That(result.IsValid).IsFalse();
        await Assert.That(result.Diagnostics.Any(x => x.Message.Contains("strict-detection must be true or false", StringComparison.Ordinal))).IsTrue();
    }

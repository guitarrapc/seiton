...
// Parse command line args: if fixture name(s) provided, only dump those
...
foreach (var yamlPath in Directory.EnumerateFiles(errDir, "*.yaml").OrderBy(p => p, StringComparer.Ordinal))
{
    ...
    if (filterNames.Count > 0 && !filterNames.Contains(name)) continue;
    ...
    // If filtering specific fixtures, show detail
    if (filterNames.Count > 0 || unmatchedExpected.Count > 0 || extraSeiton.Count > 0)
    {
        Console.WriteLine($"\n## {name} (matched={matchedCount}/{expectations.Count} extra={extraSeiton.Count})");
        Console.WriteLine("### Seiton output:");
        foreach (var line in seitonLines)
            Console.WriteLine($"  {line}");
        Console.WriteLine("### Expected (.out):");
        foreach (var (pattern, isRegex) in expectations)
            Console.WriteLine($"  {(isRegex ? "/" + pattern + "/" : pattern)}");
        if (unmatchedExpected.Count > 0)
        {
            Console.WriteLine("### MISS:");
            foreach (var (pattern, isRegex) in unmatchedExpected)
                Console.WriteLine($"  {(isRegex ? "/" + pattern + "/" : pattern)}");
        }
        if (extraSeiton.Count > 0)
        {
            Console.WriteLine("### EXTRA:");
            foreach (var line in extraSeiton)
                Console.WriteLine($"  {line}");
        }
    }
}

Console.WriteLine("=== Actionlint Compatibility Summary ===");
Console.WriteLine($"Fixtures: {fullyMatched}/{totalFixtures} fully matched");
Console.WriteLine($"Expected lines: {totalMatched}/{totalExpected} matched ({(totalExpected > 0 ? totalMatched * 100 / totalExpected : 0)}%)");
Console.WriteLine($"Extra seiton lines: {totalExtra}");
...
static string FindRepoRoot()
{
    ...
    // fallback: walk from current directory
